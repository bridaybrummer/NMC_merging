...
stata2script<- function(excel_load){
...
data21 <- data20 %>%
  mutate(Year_symptoms = as_year(symptom_date),
...
data_dup26 <- data_dup25 %>%
  mutate(Back_capture = case_when(
    ...
    as_date(diagnosis_date) > as_date("2023-05-01") ~ "Current",
    as_date(diagnosis_date )< (as_date("2023-05-01") - days(14)) ~ "Back capture",
    as_date(diagnosis_date) %in%  (as_date("2023-05-01") - days(14)) : (as_date("2023-05-01")) ~ "Delayed",
    TRUE ~ "unkown"
  ))
...
data_dup27<- data_dup26%>%mutate(prov_ =sapply(strsplit(province, " "), function(x) x[1]),
                                 age = patient_age)
